...
# Modo simulador / real
MODO_SIMULADOR = False  # Por defecto: False (modo diagnóstico/real), True (modo simulador)

# Variables de estado para la UI
_modo_actual = 'diagnosticar' if not MODO_SIMULADOR else 'simulador'
_puerto_elm327 = None
...
def get_config():
    """Retorna la configuración actual de la app."""
    global _buscando_elm327, _tiempo_inicio_busqueda
    
    # Si está buscando, verificar si ya pasó 1 minuto
    if _buscando_elm327 and _tiempo_inicio_busqueda is not None:
        tiempo_transcurrido = time.time() - _tiempo_inicio_busqueda
        if tiempo_transcurrido > 60:  # 1 minuto = 60 segundos
            _buscando_elm327 = False  # Detener búsqueda automáticamente
            _tiempo_inicio_busqueda = None
            print(f"[Config] Búsqueda de ELM327 finalizada tras {tiempo_transcurrido:.1f} segundos")
...
@app.route('/api/consumo-inteligente', methods=['GET'])
...
def get_codigos():
    from pids.codigos import nombre_codigos, codigos_db
    try:
    ...
    if codigos:
        try:
            from utils.decodificadores import decodificar_pid as _dec
            if MODO_SIMULADOR:
...
@app.route('/api/estado-motor', methods=['GET'])
...
@app.route('/api/protocolo/escanear', methods=['GET'])
